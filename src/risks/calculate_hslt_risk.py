...
def diversification_hslt_risk(aggregation_tree_enriched: pd.DataFrame) -> pd.DataFrame:
    ...
    data_id = ['HSLT_MOR_SCR_N', 'HSLT_LON_SCR_N', 'HSLT_DIS_SCR_N',
               'HSLT_LAP_SCR_N', 'HSLT_EXP_SCR_N', 'HSLT_REV_SCR_N', 'HSLT_SCR_N',
                'HSLT_MOR_SCR_G', 'HSLT_LON_SCR_G', 'HSLT_DIS_SCR_G',
                'HSLT_LAP_SCR_G', 'HSLT_EXP_SCR_G', 'HSLT_REV_SCR_G', 'HSLT_SCR_G']
    ...
    values_dict['HSLT_DIV_N'] = values_dict.get('HSLT_SCR_N', 0) - (values_dict.get('HSLT_MOR_SCR_N', 0) + values_dict.get('HSLT_LON_SCR_N', 0) +
                                                              values_dict.get('HSLT_DIS_SCR_N', 0) + values_dict.get('HSLT_LAP_SCR_N', 0) + 
                                                              values_dict.get('HSLT_EXP_SCR_N', 0) + values_dict.get('HSLT_REV_SCR_N', 0))
    values_dict['HSLT_DIV_G'] = values_dict.get('HSLT_SCR_G', 0) - (values_dict.get('HSLT_MOR_SCR_G', 0) + values_dict.get('HSLT_LON_SCR_G', 0) +
                                                              values_dict.get('HSLT_DIS_SCR_G', 0) + values_dict.get('HSLT_LAP_SCR_G', 0) + 
                                                              values_dict.get('HSLT_EXP_SCR_G', 0) + values_dict.get('HSLT_REV_SCR_G', 0))    
...
def calculate_hslt_risk_all_subrisks(input_file: str) -> pd.DataFrame:
    ...
    aggregation_tree_hslt_mor_enriched = calculate_hslt_mortality_risk(data_id_enriched_hslt)
    aggregation_tree_hslt_lon_enriched = calculate_hslt_longevity_risk(data_id_enriched_hslt)
    aggregation_tree_hslt_dis_enriched = calculate_hslt_disability_risk(data_id_enriched_hslt)
    aggregation_tree_hslt_lap_enriched = calculate_hslt_lapse_risk(data_id_enriched_hslt)
    aggregation_tree_hslt_exp_enriched = calculate_hslt_expense_risk(data_id_enriched_hslt)
    aggregation_tree_hslt_rev_enriched = calculate_hslt_revision_risk(data_id_enriched_hslt)
    aggregation_tree_hslt_total_enriched = calculate_total_hslt_risk(data_id_enriched_hslt)
    
    # Combine all hslt risk components into a single DataFrame
    aggregation_tree_hslt_all_risks = pd.concat([aggregation_tree_hslt_mor_enriched,
                                                 ...
                                                 aggregation_tree_hslt_total_enriched], ignore_index=True)
    ...
    aggregation_tree_hslt_all_risks = pd.concat([aggregation_tree_hslt_all_risks, diversification_hslt_df], ignore_index=True)  
