...
def fetch_lookup_tables() -> (
    ...
    """
    try:
        cdr_lgd_factors_response = (
            supabase.table("CDR_lgd_factors").select("*").execute()
        )
...
def get_pd_lgd(aggregated_sne: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    pd_lgd = (
        aggregated_sne.groupby("PD_weighted", dropna=True, as_index=False)
        .agg(sum_lgd=("LGD", "sum"))
...
def calculate_pd(
    row: pd.Series, rating_pd_dict: Dict[str, float], ratio_pd_df: pd.DataFrame
) -> float:
    """
    ...
    if pd.notnull(rating) and pd.notnull(scr_ratio):
        raise ValueError(
            f"Both Rating and SCR Ratio are filled (Rating: {rating}, SCR Ratio: {scr_ratio})"
        )
...
def calculate_risk_adjusted_collateral(row: pd.Series) -> float:
    ...
    if type_of_exposure in [
        ...
        if is_simplified:
            # Simplified approach
            return (
                COLLATERAL_ADJUSTMENT_3RD_PARTY_SIMPL * market_value_collateral
                if is_third_party
                else COLLATERAL_ADJUSTMENT_SIMPL * market_value_collateral
            )
        else:
            # Standard approach
            adjusted_value = market_value_collateral - collateral_adjustment
            return (
                COLLATERAL_ADJUSTMENT_3RD_PARTY * adjusted_value
                if is_third_party
                else COLLATERAL_ADJUSTMENT * adjusted_value
            )
...
def calculate_lgd(exposure_row: pd.Series, lgd_factors_df: pd.DataFrame) -> float:
    ...
    factors = lgd_factors_df[
        ...
        & (
            lgd_factors_df["reinsurance_tied_up"].astype(str).str.lower()
            == str(reinsurance_tied_up).lower()
        )
    ]
    ...
    try:
        if type_of_exposure in [
            ...
            lgd = max(
                factor["LOSS_FACTOR"]
                * (net_market_value + factor["ALPHA"] * risk_mitigating_effect)
...
def aggregate_by_parent_group(counterparty_exposures: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_columns = [
        col for col in required_columns if col not in counterparty_exposures.columns
    ]
    ...
    try:
        ...
        # Perform aggregation
        aggregated = (
            counterparty_exposures.groupby("Parent Group")
            .agg(
                LGD=("LGD", "sum"),
                PD_weighted=(
                    "PD",
                    lambda x: (
                        ...
                        if counterparty_exposures.loc[x.index, "LGD"].sum() > 0
                        else np.nan
                    ),
                ),
            )
        ...
        logger.info(
            f"Aggregated {len(counterparty_exposures)} exposures into {len(aggregated)} parent groups"
        )
        return aggregated
...
def calculate_v_inter(
    distinct_pd: np.ndarray, sum_lgd: np.ndarray
    ...
    """
    v_inter_terms = (1.5 * distinct_pd * (1 - distinct_pd) * sum_lgd**2) / (
        2.5 - distinct_pd
    )
...
def calculate_pool_sii_ratio(
    row: pd.Series, pool_ratio_dict: Dict[str, float]
) -> float:
    """
...
def calculate_pool_metrics(
    df: pd.DataFrame, lgd_factors_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    if missing_cols:
        raise ValueError(
            f"Missing required columns for pool calculation: {missing_cols}"
        )
    ...
    mask_nonzero_eof = (
        (pool_a_mask)
        & (df["aggregated_EOF/Pool SII Ratio"].notna())
        & (df["aggregated_EOF/Pool SII Ratio"] != 0)
    )
    mask_zero_eof = (
        (pool_a_mask)
        & (df["aggregated_EOF/Pool SII Ratio"].notna())
    ...
    df.loc[mask_nonzero_eof, "Q"] = np.exp(
        -0.15
        * np.minimum(
            1.96,
            (
            ...
            + df.loc[mask_nonzero_eof, "aggregated_SoR * Pool SII Ratio"]
            - 1,
        )
    )

    # Calculate Q for zero EOF/Pool SII Ratio
    df.loc[mask_zero_eof, "Q"] = np.exp(
        -0.15
        * np.minimum(1.96, df.loc[mask_zero_eof, "aggregated_SoR * Pool SII Ratio"] - 1)
    )
    ...
    df.loc[pool_a_mask, "LGD"] = (
        df.loc[pool_a_mask, "Q"] * df.loc[pool_a_mask, "LGD_Base"]
    )
...
def calculate_pool_type_bc_lgd(row: pd.Series, lgd_factors_df: pd.DataFrame) -> float:
    ...
    second_term = (
        pool_rm_contribution * risk_mitigating_effect
        if not pd.isna(pool_rm_contribution)
        else 0
    )
...
def transform_to_aggregation_tree(aggregated_sne: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.rename(
        columns={"Parent Group": "PARENT_GROUP", "LGD": "LGD", "PD_weighted": "PD"}
    )
    ...
    melted["NODE_ID"] = melted.apply(
        lambda x: f"{x['variable']}_{x['index']:02d}", axis=1
    )
...
def calculate_cpty_type1(
    data_id_enriched: pd.DataFrame,
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, float, float, float, pd.DataFrame]:
    """
    ...
    try:
        # Fetch lookup tables
        cdr_lgd_factors, cdr_rating_pd, cdr_ratio_pd, cdr_pool_ratio = (
            fetch_lookup_tables()
        )

        rating_pd_dict = dict(zip(cdr_rating_pd["Rating"], cdr_rating_pd["PD"]))
        pool_ratio_dict = dict(
            zip(cdr_pool_ratio["Rating"], cdr_pool_ratio["Pool SII Ratio"])
        )
        ...
        df["PD"] = df.apply(
            lambda row: calculate_pd(row, rating_pd_dict, cdr_ratio_pd), axis=1
        )
...
if __name__ == "__main__":
    ...
    supabase_data_importer = SupabaseDataImporter()
    data_id_enriched = supabase_data_importer.run_import(input_file)
