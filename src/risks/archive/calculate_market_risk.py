...
def read_market_risk_input(input_path: str) -> pd.DataFrame:
    ...
    try:
        return pd.read_excel(
            input_path, sheet_name="MarketR", header=0, engine="pyxlsb"
        )
...
def aggregate_tree(
    ...
    def calculate_dnav(children):
        try:
            base_assets = children[
                (children["BS_TYPE"] == "asset") & (children["SCENARIO"] == "BC")
            ]["VALUE"].sum()

            base_liab = children[
                (children["BS_TYPE"] == "liab") & (children["SCENARIO"] == "BC")
            ]["VALUE"].sum()

            shocked_assets = children[
                (children["BS_TYPE"] == "asset") & (children["SCENARIO"] == "SH")
            ]["VALUE"].sum()

            shocked_liab = children[
                (children["BS_TYPE"] == "liab") & (children["SCENARIO"] == "SH")
            ]["VALUE"].sum()
    ...
    results["VALUE"] = results["VALUE"].astype(
        object
    )  # Allow mixed types (numeric and string)
    ...
    while results["VALUE"].isnull().any():
        ...
        if iteration_count > max_iterations:
            print(
                f"Maximum iterations ({max_iterations}) reached. Possible circular dependency in tree."
            )
            break
        ...
        for node_id in results.index[results["VALUE"].isnull()]:
            ...
            if len(children) == 0:
                if node["AGGREGATION_METHOD_CD"] == "external":
                    ...
                    results.loc[node_id, "VALUE"] = (
                        0  # Default value for non-external leaf nodes
                    )
                continue
            ...
            try:
                if method == "sum":
                    results.loc[node_id, "VALUE"] = results.loc[
                        children["NODE_ID"], "VALUE"
                    ].sum()
...
def process_output_mapping(
    results: pd.DataFrame, mapping_template_path: str
) -> pd.DataFrame:
    """
    ...
    for col in value_columns:
        mapping[col] = mapping[col].map(
            lambda x: results.loc[x, "VALUE"] if x in results.index else x
        )

    return mapping


# This function overlaps the aggregation_tree one - please verify if it is the intention
...
def read_data_id_values(worksheet: str = "MarketR") -> pd.DataFrame:
    ...
    """
    response = (
        supabase.table("data_id").select("*").eq("WORKSHEET", worksheet).execute()
    )
    return pd.DataFrame(response.data)
...
def main(run_id: str = None):
    ...
    data_id_values["VALUE"] = data_id_values.apply(
        lambda row: (
            get_cell_value(market_risk_input, row["RC_CODE"])
            if row["WORKSHEET"] == "MarketR" and pd.notnull(row["RC_CODE"])
            else None
        ),
        axis=1,
    )
    ...
    for _, node in tree_data[
        tree_data["AGGREGATION_METHOD_CD"] == "external"
    ].iterrows():
        matching_data = data_id_values[data_id_values["DATA_ID"] == node["NODE_ID"]]
        if not matching_data.empty:
            tree_data.loc[tree_data["NODE_ID"] == node["NODE_ID"], "VALUE"] = (
                matching_data["VALUE"].iloc[0]
            )
