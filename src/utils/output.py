...
def normalize_column_names(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "VALUE" not in df.columns:
        if "Value" in df.columns:
            ...
            raise ValueError(
                "The input DataFrame must contain a column named 'VALUE', 'Value', or 'value'."
            )
...
if __name__ == "__main__":
    ...
    dataframe = supabase_data_importer.run_import(
        "/workspaces/SolvMate/input/02.01_SAS_Input_MarketR.xls"
    )
