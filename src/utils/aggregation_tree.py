...
class AggregationTree:
    ...
    def aggregate_tree(self, aggregation_tree, data_id_enriched, aggregation_tree_id):
        ...
        for index, row in filtered_tree.iterrows():
            node_id = row["NODE_ID"]
            filtered_tree.at[index, "VALUE"] = self._calculate_value(
                node_id, filtered_tree, data_id_enriched
            )
        return filtered_tree

    def _calculate_value(self, node_id, aggregation_tree, data_id_enriched):
        ...
        for _, child in child_nodes.iterrows():
            child_value = self._calculate_value(
        ...
        if aggregation_method_cd == "sum":
            ...
            calculated_value = max(
                (base_case_assets - base_case_liabilities)
            ...
            impostor_base_id = aggregation_tree.loc[
                aggregation_tree["NODE_ID"] == node_id, "MAX_SCENARIO_BASE"
            ].iat[0]

            # find children of impostor base node
            impostor_children = aggregation_tree.loc[
                aggregation_tree["PARENT_NODE_ID"] == impostor_base_id
            ]
            ...
            if impostor_children.empty:
                ...
                for idx, child_row in impostor_children.iterrows():
                    ...
                    if pd.isna(aggregation_tree.at[idx, "VALUE"]):
                        aggregation_tree.at[idx, "VALUE"] = self._calculate_value(
                            child_node_id, aggregation_tree, data_id_enriched
                        )
                ...
                winning_scenarios = set(
                    impostor_children.loc[
                        impostor_children["VALUE"] == max_val_impostor, "SCENARIO"
                    ]
                    .dropna()
                    .astype(str)
                    .tolist()
                ...
                original_children = aggregation_tree.loc[
                    aggregation_tree["PARENT_NODE_ID"] == node_id
                ]
                ...
                if original_children.empty:
                    ...
                    filtered = original_children[
                        original_children["SCENARIO"]
                        .astype(str)
                        .isin(winning_scenarios)
                    ]
    ...
    def read_aggregation_tree(self, tree_id: str) -> pd.DataFrame:
        """
