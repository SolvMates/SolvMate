...
def check_cell_value_format(cell_value: str) -> bool:
    ...
    if re.match(pattern1, cell_value) or re.match(pattern2, cell_value) or re.match(pattern3, cell_value) or re.match(pattern4, cell_value):
        return True
    return False
...
def read_excel_cells(file_path: str):
    ...
    results = pd.DataFrame(columns=['QRT_ID', 'QRT_NAME', 'RC_CODE', 'CELL_REFERENCE', 'DATA_ID','ID'])
        ...
        for row in range(df_sheet.shape[0]):  # Zeilen
            for col in range(df_sheet.shape[1]):  # Spalten
                ...
                if check_cell_value_format(str(cell_value)):
                    ...
                    cell_position = f'{chr(65 + col)}{row + 2}'  # +2 für menschliche Zählweise
...
def upsert_data_to_supabase(dataframe: pd.DataFrame, table_name: str, unique_column: str):
    for index, row in dataframe.iterrows():
        ...
        existing_record = supabase.table(table_name).select('*').eq(unique_column, unique_value).execute()

        if existing_record.data:
            # Datensatz existiert, also aktualisieren wir ihn
            supabase.table(table_name).update(row.to_dict()).eq(unique_column, unique_value).execute()
...
if __name__ == "__main__":
    goal_dataframe = read_excel_cells(file_path="/workspaces/SolvMate/templates/Output_ERGO.xlsx",)
    #create excel file
